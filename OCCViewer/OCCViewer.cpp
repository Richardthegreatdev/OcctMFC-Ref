...
// OCCViewer.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(COCCViewerApp, CWinAppEx)
	...
	ON_COMMAND(ID_FILE_OPEN, &COCCViewerApp::OnFileOpen)
END_MESSAGE_MAP()


// COCCViewerApp construction
...
// The one and only COCCViewerApp object

COCCViewerApp theApp;


// COCCViewerApp initialization
...
int COCCViewerApp::SelectTemplate()
{
	int res = 1;

	res = AfxMessageBox(L"Start OCAF Application", MB_YESNO);
	...
	if (res == IDYES)
	{
		...
		pDocTemplate = new CSingleDocTemplate(
			IDR_MAINFRAME,
			RUNTIME_CLASS(ParametricDoc),	 // Occ Document
			RUNTIME_CLASS(CMainFrame),       // main SDI frame window
			RUNTIME_CLASS(OCAFView));
		if (!pDocTemplate)
			return FALSE;
		AddDocTemplate(pDocTemplate);
	}
...
// COCCViewerApp message handlers
...
// COCCViewerApp customization load/save methods
...
void COCCViewerApp::OnFileOpen()
{
	CFileDialog aDlg(TRUE, NULL, NULL, OFN_FILEMUSTEXIST | OFN_HIDEREADONLY,
		L"OCC Document(Binary) (*.cbf)|*.cbf|OCC Document(Xml) (*.xml)|*.xml||");
